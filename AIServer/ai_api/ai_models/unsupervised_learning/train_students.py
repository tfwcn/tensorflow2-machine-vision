...
from ai_api.ai_models.unsupervised_learning.teacher_dataset import DataGenerator as TeacherDataGenerator
...
def train():
  ...
  data_generator_train = TeacherDataGenerator(image_path=trainData,
    classes_path=classesFile, batch_size=batchSize, anchors=anchors,
    image_wh=(416, 416), image_random=True, flip=True)
  data_set_train = data_generator_train.GetDataSet()
  data_generator_val = DataGenerator(image_path=valData, label_path=valLabels,
    classes_path=classesFile, batch_size=1, anchors=anchors,
    image_wh=(416, 416), label_mean=False, image_random=False, flip=False)
  ...
  print('Train on {} samples, val on {} samples, with batch size {}.'.format(data_generator_train.image_num, data_generator_val.labels_num, batchSize))
  print('开始训练')
